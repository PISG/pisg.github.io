...
## 2026-09-21 — docs redesign and an Upgrading chapter

### Added

- `/docs/#upgrading`: **Upgrading to 1.0a** — what stays the same and what changes, what to back up,
  a four-step side-by-side upgrade, how to get the pre-1.0a page back, the new exit status for
  scripts and cron jobs, emptying `CacheDir`, and the opt-in extras. Generated by
  `tools/make-docs.py` from the release notes.

### Changed

- `/docs/` has the site's top bar, like every other page; the sidebar sits under it and the logo
  block is gone from the sidebar.
- The content column uses the full window instead of a 900 px strip. Prose is held to a reading
  measure; option cards, code and the console take the width. On wide screens each option card puts
  its name, purpose and default beside the description.
- More readable text: 16 px body, taller line height, a softer reading colour (`--text2`) for
  paragraphs, underlined links in running text, and larger chapter headings with a numbered kicker.
- New docs hero, in the home page's style, with four entry points (setting up, upgrading, options,
  example config).
- The sidebar scrolls itself to keep the section you are reading in view.
